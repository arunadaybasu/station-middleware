...
router.get('/market/sell/lunc', async function(req, res, next) {

  await connectToBinance();
  
  await binanceApiClient.newOrder(
  // await binanceApiClient.newOrderTest(
    // 'LUNCUSDT', 'SELL', 'LIMIT',
    'LUNCUSDT', 'BUY', 'MARKET',
    {
      'newClientOrderId': 'skdhaskdh',
      // 'quantity': '100.0',
      'quoteOrderQty': '5',
      // 'price': '0.000075',
      // 'timeInForce': 'GTC'
...
router.get('/market/trades/all', async function(req, res, next) {

  await connectToBinance();
  
  await binanceApiClient.myTrades(
    'LUNCUSDT',
    {
      'orderId': 596538473
    }
...
router.get('/market/orders/all', async function(req, res, next) {

  await connectToBinance();
  
  await binanceApiClient.allOrders(
    'LUNCUSDT',
    // {
    //   'orderId': 596538473
    // }
...
router.get('/market/account-info', async function(req, res, next) {

  await connectToBinance();
  
  await binanceApiClient.account({})
  .then(response => {
    console.log(response);
    res.send(response.data);
  })
...
router.get('/swapquote', async function(req, res, next) {
  ...
  binanceApiClient.bswapRequestQuote('USDT', 'SHIB', 101)
  .then(response => {
    console.log(response.data);
    res.send(response.data);
  })
